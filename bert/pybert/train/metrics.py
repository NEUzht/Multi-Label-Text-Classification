...
class Precision(Metric):
    ...
    def __call__(self, logits, target):
        ...
        if self.search_thresh:
            thresh,precision = self.thresh_search(y_prob = self.y_pred)
            print(f"Best thresh: {thresh:.4f} - Precision: {precision:.4f}")
            self.best_thresh = thresh
        else:
            self.y_pred = np.array(self.y_pred) >= self.thresh
            precision = precision_score(self.y_true, self.y_pred, average='samples')
    ...
    def value(self):
        # self.y_pred = np.array(self.y_pred) >= self.thresh
        acc = precision_score(self.y_true, self.y_pred, average='samples')
        return acc 
    
    def print_thresh(self):
        return self.best_thresh
...
class Recall(Metric):
    ...
    def __call__(self, logits, target):
        ...
        self.y_true = target
        if self.search_thresh:
            thresh,recall = self.thresh_search(y_prob = self.y_pred)
            print(f"Best thresh: {thresh:.4f} - Precision: {recall:.4f}")
            self.best_thresh = thresh
        else:
            self.y_pred = np.array(self.y_pred) >= self.thresh
            recall = recall_score(self.y_true, self.y_pred, average='samples')
    ...
    def value(self):
        ...
        # self.y_pred = np.array(self.y_pred) >= self.thresh
        acc = recall_score(self.y_true, self.y_pred, average='samples')
        return acc
...
class F1Score(Metric):
    ...
    def thresh_search(self,y_prob):
        ...
        for threshold in tqdm([i * 0.01 for i in range(100)], disable=True):
            self.y_pred = y_prob > threshold
            score = self.f1_value()
            precision_ = self.precision_value()
            recall_ = self.recall_value()
            accuracy_ = accuracy_score(self.y_true, self.y_pred)
            if score > best_score:
                best_threshold = threshold
                best_score = score
        ...
        return best_threshold,best_score,precision,recall,accuracy
    ...
    def __call__(self,logits,target):
        ...
        if self.task_type == 'binary':
            if self.thresh and self.search_thresh == False:
                ...
                thresh,f1,precision,recall,accuracy = self.thresh_search(y_prob = y_prob)
                print(f"Best thresh: {thresh:.4f} - F1 Score: {f1:.4f} - Precision: {precision:.4f} - Recall: {recall:.4f}- accuracy: {accuracy:.4f}")
                self.best_thresh = thresh
    ...
    def value(self):
        '''
         计算指标得分   
         '''
        self.y_pred = self.prob1 > self.thresh
