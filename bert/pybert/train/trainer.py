...
class Trainer(object):
    ...
    def valid_epoch(self,data):
        ...
        # weight = [1365,334,547,1546,1035,2076,746,174]
        # total_sum = sum(weight)
    
        # weight = [(total_sum - x) / total_sum for x in weight]
        # loss = focal_loss_multilabel(targets=targets,outputs = logits,weight = weight)
        ...
        if self.epoch_metrics:
            for metric in self.epoch_metrics:
                ...
                value = metric.value()
                if metric.name() == "f1":
                    self.result[f'valid_{metric.name()}_f1_thresh'] = metric.print_thresh()
                if value:
    ...
    def train_epoch(self,data):
        ...
        for step,  batch in enumerate(data):
            ...
            logits = self.model(ids, mask, token_type_ids)
            loss = self.criterion(output=logits,target=targets)
            # weight = [5395,1383,8149,728,2302,4045,6331,2949]
            # total_sum = sum(weight)
            # weight = [(total_sum - x) / total_sum for x in weight]
            # loss = focal_loss_multilabel(targets=targets,outputs = logits,weight = weight)
            ...
            if self.batch_metrics:
                for metric in self.batch_metrics:
                    metric(logits = logits,target = targets)
        ...
        if self.epoch_metrics:
            for metric in self.epoch_metrics:
                ...
                if metric.name() == "f1":
                    self.result[f'{metric.name()}_f1_thresh'] = metric.print_thresh()
                if value:      
