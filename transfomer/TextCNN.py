...
class TextCNN(nn.Module):
    def __init__(self, kernel_num=2, kernel_size=[2,3,4], vocab_size=0, embed_dim=128, dropout=0.2, class_num=8):
        super().__init__()
...
if __name__ == "__main__":
    ...
    model = TextCNN(kernel_num, kernel_size, 100000, embed_dim, dropout, class_num).cuda()
    model.init_weight()
    x = torch.randint(0,100000,(8,20)).cuda()
    print(x.shape)
    y = model(x)
    print(y.shape)
    print(y)
